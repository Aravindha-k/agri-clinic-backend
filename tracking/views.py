...
class BulkLocationUploadAPI(APIView):
    ...
    def post(self, request):
        # Expecting a list of location dicts in request.data["locations"]
...
class PushLocationAPI(APIView):
    ...
    def post(self, request):
        ...
        if not workday:
            return Response({"detail": "Workday not started"}, status=400)
    ...
    def get(self, request):
        ...
        for emp in employees:
            ...
            if workday:
                ...
                if (
                    workday.last_heartbeat
                    and workday.last_heartbeat >= heartbeat_threshold
                ):
                ...
                if loc:
                    row["last_latitude"] = float(loc["latitude"])
                    row["last_longitude"] = float(loc["longitude"])
                    row["last_seen"] = loc["recorded_at"].isoformat()
        ...
        for emp in employees:
            ...
            if workday:
                ...
                if loc:
                    ...
                    props["last_seen"] = loc.recorded_at.isoformat()
...
class AdminEmployeeRouteAPI(APIView):
    ...
    def get(self, request, user_id):
        ...
        points = list(
            ...
            .order_by("recorded_at")
            .values("latitude", "longitude", "accuracy", "is_suspicious", "recorded_at")
        )
